...
long lastMsg = 0;
char msg[50], mqtt_topic[40], mqtt_server[40], calibrationStr[8];
...
void setup() {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          ...
          strcpy(calibrationStr, json["calibration"]);
  ...
  WiFiManagerParameter custom_calibration("calibration", "clibration for the PH sensor (eq 42.00)", calibrationStr, 8);
  ...
  strcpy(calibrationStr, custom_calibration.getValue());
  ...
  if (shouldSaveConfig) {
    ...
    json["calibration"] = calibrationStr;
...
void loop() {
  ...
  char buffer_str[7];
  float calibration = atof(calibrationStr);
  int sensorValue = 0; 
  ...
  dtostrf(phValue, 4, 6, buffer_str);  //4 is mininum width, 6 is precision
